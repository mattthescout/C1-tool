    ...
    if (buff[0] == CMD_ERROR)
    {
        ...
        case CMD_GET_UID:
            ...
            for (size_t i = 0; i < part_msg_cnt; i++)
            {
                if (i == 0)
                {
                    memcpy(p_ndef_msg, p_bitmap_all, temp_lenght);
                }
                else if (i == (part_msg_cnt - 1))
                {
                    memcpy(ndef_msg, (p_bitmap_all + (i * temp_lenght)), (part_msg_length - 1));
                    ndef_msg[(part_msg_length)] = 0xFE;
                }
                else
                {
                    memcpy(ndef_msg, (p_bitmap_all + (i * part_msg_length)), part_msg_length);
                }
            ...
            /*
                        ...
                        for (uint8_t i = 0; i <= (length); i++)
                        {
                            ...
                            memcpy((cmd + 4), ndef_msg + (i * (msg_len - 4)), (msg_len - 4));
